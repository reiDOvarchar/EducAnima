...
include "notaController.class.php";
...
if ($action == 'insertNota') {
    $nota = filter_input(INPUT_POST, 'nota');
    $ret = $notaController->insertNota($nota);
}

if ($action == 'mostrarNota') {
    $item = filter_input(INPUT_POST, 'item');
    $ret = $notaController->mostrarNotaDesenho($item);
}

if ($action == 'getIdDesenho') {
    $ret = $notaController->getIdDesenho();
}
